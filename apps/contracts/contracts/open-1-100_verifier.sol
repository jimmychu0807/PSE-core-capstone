...
contract PlonkVerifier {
  ...
  uint256 constant Qmx =
    16784758587654484488756149244054446994937766695835781204986386714213328100464;
  uint256 constant Qmy =
    15841712202734707466578430453173187574942070438472050961904480899236235099037;
  uint256 constant Qlx =
    18065516747107636167596507506979626429317174256622650562293093939827098179739;
  uint256 constant Qly =
    12216630469456080374100929065165678781196301151499802076772654524576936668782;
  uint256 constant Qrx =
    15783585639015086085829062722149205473859094623828952240419682490768909805489;
  uint256 constant Qry =
    15464386668881943913466822442347200990410920720481754569782910134764352107285;
  uint256 constant Qox =
    10600313699688642361932406989448667517852288022884501306443266353421943566796;
  uint256 constant Qoy =
    3672822945861320114007435524574688696939905867716908401291881837629025934925;
  uint256 constant Qcx =
    20027817867110951510710183019951646035497353363697965986850360246380656020106;
  uint256 constant Qcy =
    20684002179155390375970343110228988061106119183984831156540761562624275787123;
  uint256 constant S1x =
    3197666535811647756231832602968098861868031135026507571582906434274976002591;
  uint256 constant S1y =
    20430110100121092525926548379772338247435973132192538070661983593226615115723;
  uint256 constant S2x =
    12590433488128312413009291220326554605450220431752391295467256112901855487487;
  uint256 constant S2y =
    18049595614517052052563958083376758085050843756791095576410171541275706078938;
  uint256 constant S3x =
    15594854580594066927552458186267847679933698834532254258261397749297213722233;
  uint256 constant S3y =
    13617326068303162355256651437249804287057339068953717354884925118618637842250;
  ...
  function verifyProof(
    ...
    assembly {
      ...
      function calculatePI(pMem, pPub) {
        ...
        pl := mod(
          add(sub(pl, mulmod(mload(add(pMem, pEval_l3)), calldataload(add(pPub, 64)), q)), q),
          q
        )

        pl := mod(
          add(sub(pl, mulmod(mload(add(pMem, pEval_l4)), calldataload(add(pPub, 96)), q)), q),
          q
        )
